...
4. **Apa yang terjadi secara otomatis?**
   - Skrip ini (`scripts/emergency-lockdown.mjs`) akan membuat satu file *Backup SQL* khusus di server Anda lengkap dengan stempel waktu dan hash integritas SHA-256 (`.sha256`).
   - Skrip ini merubah pengaturan Database untuk memaksa 25 halaman utama website berubah menjadi mode *Maintenance* (Pengunjung akan melihat layar perbaikan, bukan halaman asli).
   - Skrip ini masuk ke file `.env` dan merusak/mengacak sandi `JWT_SECRET` secara otomatis. Ini akan memutus koneksi peretas secara paksa.

5. **Langkah Wajib Lanjutan:**
   Segera **Restart** aplikasi Nuxt/Node.js Anda (via panel PM2 atau *dashboard hosting* Railway) agar *password secret* (`JWT_SECRET`) yang baru tersebut dibaca oleh sistem. Mulai saat ini, tidak ada satu orang pun yang bisa mengakses panel admin menggunakan sesi (*cookies/token*) yang lama.
...
3. **Apa yang terjadi secara otomatis?**
   - Sistem akan menandai semua akun pengurus (Admin Wilayah, BGKP, Komsos, dll - kecuali Super Admin) sebagai **Wajib Reset** (`requires_password_reset = 1`).
