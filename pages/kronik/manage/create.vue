    ...
    <div class="container mx-auto px-4 max-w-4xl">
      ...
      <div class="mb-6">
        ...
        <p class="text-gray-600 mt-1">
          Isi form berikut untuk menambahkan kronik kegiatan paroki
        </p>
      </div>
      ...
      >
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
          <div>
            ...
            <select
              ...
              required
              :disabled="isCategoryDisabled || loadingUser"
            ...
            >
              <option value="">
                {{ loadingUser ? "Memuat..." : "Pilih Kategori" }}
              </option>
          ...
          <div>
            ...
            <select
              ...
              :disabled="!form.category_id || loadingSections"
              class="w-full border border-gray-300 rounded-lg px-4 py-2 focus:ring-2 focus:ring-paulus-blue focus:border-transparent disabled:bg-gray-100 disabled:cursor-not-allowed"
            >
              <option value="">
                {{
            ...
            <p
              v-if="
                form.category_id && !loadingSections && sections.length === 0
              "
              class="text-sm text-amber-600 mt-1"
            >
              Kategori ini belum memiliki bagian. Anda dapat melanjutkan tanpa
              memilih bagian.
            </p>
          </div>
        </div>
              ...
              <div class="space-y-3">
                ...
                <div>
                  ...
                  <p class="text-xs text-gray-500 mt-1">
                    ...
                    <span v-else
                      >Upload 1 foto utama untuk ditampilkan sebagai
                      thumbnail</span
                    >
                  </p>
                  ...
                  >
                    <img
                      :src="resolveKronikImagePath(form.featured_image)"
                      alt="Preview"
                ...
                <div>
                  ...
                  <input
                    ...
                    :disabled="
                      uploadingGallery ||
                      (form.gallery && form.gallery.length >= 5)
                    "
                  ...
                  >
                    ...
                    >
                      <img
                        :src="resolveKronikImagePath(img)"
                        alt="Gallery"
        ...
        <!-- AI Generate Narasi -->
        <div
          class="border-t pt-6 bg-gradient-to-r from-blue-50 to-purple-50 p-6 rounded-lg"
        >
          <div class="flex items-start justify-between mb-4">
            <div>
              ...
              <p class="text-sm text-gray-600">
                Buat narasi kronik otomatis berdasarkan data 5W1H yang sudah
                diisi. Minimal isi What, When, dan Where terlebih dahulu.
              </p>
            </div>
          </div>
          ...
          <p
            v-if="!form.what_title || !form.when_date || !form.where_location"
            class="text-xs text-amber-600 mt-2"
          >
            ⚠️ Minimal isi What (Judul), When (Tanggal), dan Where (Lokasi)
      ...
      <div
        v-if="showNarasiModal"
        class="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4"
      >
        <div
          class="bg-white rounded-lg max-w-3xl w-full max-h-[80vh] overflow-hidden shadow-2xl"
        >
          <!-- Modal Header -->
          <div
          ...
          >
            ...
            <p class="text-sm text-purple-100 mt-2">
              Review narasi di bawah dan klik "Salin ke Form" untuk
              menggunakannya
            </p>
          </div>
          ...
          <div class="p-6 overflow-y-auto max-h-[calc(80vh-200px)]">
            <div class="bg-gray-50 rounded-lg p-6 border border-gray-200">
              <div
                class="prose prose-sm max-w-none text-gray-800 leading-relaxed whitespace-pre-line"
              >
                {{ generatedNarasi }}
              </div>
            </div>
...
watch(
  () => form.category_id,
  async (newCategoryId: string | number) => {
    ...
    if (user.value) {
      ...
      if (user.value.organization_type) {
        const orgTypeMap: Record<string, number> = {
          gereja: 1, // Gereja
          dpp: 2, // DPP
          bgkp: 3, // BGKP   <--- LETAKNYA DI SINI
          wilayah: 4, // Wilayah
          lingkungan: 5, // Lingkungan
        };
        selectedCategoryId =
          orgTypeMap[user.value.organization_type.toLowerCase()] || null;
      }
      ...
      if (!selectedCategoryId) {
        ...
        selectedCategoryId =
          categoryMap[String(user.value.user_category || "").toUpperCase()] ||
          null;
      }
...
const fetchSections = async (categoryId: number | string | null = null) => {
  ...
  try {
    ...
    const response = (await $fetch(
      `/api/kronik/sections?category_id=${categoryId}`,
    )) as any;
    ...
    if (user.value && user.value.organization_id && !isAdminRole) {
      ...
      availableSections = availableSections.filter(
        (s: Section) => s.id === user.value?.organization_id,
      );
...
const handleSubmit = async (status = "pending") => {
  ...
  try {
    ...
    const errorMsg =
      error.data?.message ||
      error.message ||
      "Gagal menyimpan kronik. Silakan coba lagi.";
    alert(errorMsg);
  } finally {
    loading.value = false;
  }
...
const generateNarasi = async () => {
  ...
  if (!form.what_title || !form.when_date || !form.where_location) {
    alert(
      "Minimal isi What (Judul), When (Tanggal), dan Where (Lokasi) terlebih dahulu untuk generate narasi.",
    );
    return;
  }
  ...
  try {
    ...
    const response = (await $fetch("/api/kronik/generate-narasi", {
      ...
      body: {
        ...
        when:
          form.when_date +
          (form.when_duration ? ` (Durasi: ${form.when_duration})` : ""),
        where:
          form.where_location +
          (form.where_address ? ` - ${form.where_address}` : ""),
    ...
    alert(
      error.data?.message ||
        error.message ||
        "Gagal generate narasi dengan AI. Pastikan API Key Gemini sudah dikonfigurasi.",
    );
  } finally {
...
const copyNarasiToForm = () => {
  ...
  if (
    !form.what_description ||
    confirm("Replace deskripsi yang ada dengan narasi AI?")
  ) {
