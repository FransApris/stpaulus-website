<template>
  <div class="min-h-screen bg-gray-50">
    ...
    <div v-if="pullState.isPulling || pullState.isRefreshing"
      ...
      <div class="flex items-center justify-center space-x-2">
        ...
        <span class="text-sm font-medium">
          {{ pullState.isRefreshing ? 'Memuat ulang...' : pullState.canRefresh ? 'Lepaskan untuk memuat ulang' : `Tarik
      ...
      <div class="bg-white rounded-xl shadow-sm border border-gray-100 p-4 mb-6">
        ...
        <div class="grid grid-cols-2 sm:grid-cols-4 gap-3">
          ...
          <div>
            ...
            <select v-model="activeFilter.wilayah_id" @change="applyFilter"
              ...
              <option v-for="w in wilayahList" :key="w.id" :value="w.id">{{ w.nama }}</option>
            </select>
          </div>
          ...
          <div>
            ...
            <select v-model="activeFilter.lingkungan_id" @change="applyFilter"
              class="w-full border border-gray-200 rounded-lg px-3 py-2 text-sm focus:ring-2 focus:ring-[#882f1d] focus:border-[#882f1d] outline-none bg-white">
              <option :value="null">Semua Lingkungan</option>
              <option v-for="l in lingkunganList" :key="l.id" :value="l.id">{{ l.nama }}</option>
            </select>
          </div>

          <!-- Filter Seksi -->
          <div>
            <label class="block text-xs font-medium text-gray-500 mb-1">Seksi</label>
            <select v-model="activeFilter.seksi_id" @change="applyFilter"
              class="w-full border border-gray-200 rounded-lg px-3 py-2 text-sm focus:ring-2 focus:ring-[#882f1d] focus:border-[#882f1d] outline-none bg-white">
              <option :value="null">Semua Seksi</option>
              <template v-for="bidang in seksiGrouped" :key="bidang.nama">
                <optgroup :label="bidang.nama">
                  <option v-for="s in bidang.seksi" :key="s.id" :value="s.id">{{ s.nama }}</option>
                </optgroup>
              </template>
            </select>
          </div>
          ...
          <div class="flex items-end">
            <label
              ...
              :class="activeFilter.is_bgkp ? 'bg-amber-50 border-amber-300' : ''">
              <input type="checkbox" v-model="activeFilter.is_bgkp" @change="applyFilter"
        ...
        <div v-if="hasActiveFilter" class="flex flex-wrap gap-2 mt-3 pt-3 border-t border-gray-100">
          <span v-if="activeFilter.wilayah_id"
            class="inline-flex items-center gap-1 bg-blue-100 text-blue-700 text-xs px-2 py-1 rounded-full font-medium">
            Wilayah: {{wilayahList.find(w => w.id === activeFilter.wilayah_id)?.nama}}
            <button @click="activeFilter.wilayah_id = null; applyFilter()" class="hover:text-blue-900">×</button>
          </span>
          <span v-if="activeFilter.lingkungan_id"
            class="inline-flex items-center gap-1 bg-green-100 text-green-700 text-xs px-2 py-1 rounded-full font-medium">
            Lingkungan: {{lingkunganList.find(l => l.id === activeFilter.lingkungan_id)?.nama}}
            <button @click="activeFilter.lingkungan_id = null; applyFilter()" class="hover:text-green-900">×</button>
          </span>
          <span v-if="activeFilter.seksi_id"
            class="inline-flex items-center gap-1 bg-purple-100 text-purple-700 text-xs px-2 py-1 rounded-full font-medium">
            Seksi: {{allSeksi.find(s => s.id === activeFilter.seksi_id)?.nama}}
            <button @click="activeFilter.seksi_id = null; applyFilter()" class="hover:text-purple-900">×</button>
          </span>
          <span v-if="activeFilter.is_bgkp"
            class="inline-flex items-center gap-1 bg-amber-100 text-amber-700 text-xs px-2 py-1 rounded-full font-medium">
            BGKP
            <button @click="activeFilter.is_bgkp = false; applyFilter()" class="hover:text-amber-900">×</button>
          </span>
        </div>
      </div>
    </div>
    ...
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-4">
      ...
      <div v-else-if="posts && posts.length > 0" ref="contentRef"
        class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        <ArticleCard v-for="post in paginatedPosts" :key="post.id" :image="post.image" image-type="url"
          :title="post.title" :description="post.excerpt" :date="post.date" :to="`/berita/${post.slug}`"
...
interface Wilayah { id: number; nama: string; }
interface Lingkungan { id: number; nama: string; }
interface Seksi { id: number; nama: string; bidang?: string; }
interface Post {
  id: number | string;
  title: string;
  excerpt: string;
...
const { data: wilayahRaw } = await useAsyncData('filter-wilayah', () =>
  $fetch<{ data: Wilayah[] }>('/api/admin/wilayah').catch(() => ({ data: [] }))
)
// Penyesuaian membaca object { data: [...] } atau array langsung
const wilayahList = computed<Wilayah[]>(() => wilayahRaw.value?.data || (wilayahRaw.value as unknown as Wilayah[]) || [])

const { data: lingkunganRaw } = await useAsyncData('filter-lingkungan', () =>
  $fetch<Lingkungan[]>('/api/lingkungan').catch(() => [])
)
const lingkunganList = computed<Lingkungan[]>(() => lingkunganRaw.value || [])

const { data: allSeksiRaw } = await useAsyncData('filter-seksi', () =>
  $fetch<Seksi[]>('/api/seksi').catch(() => [])
)
const allSeksi = computed<Seksi[]>(() => allSeksiRaw.value || [])
...
const { data: posts, pending, error, refresh } = await useAsyncData<Post[]>('posts',
  async () => {
    try {
...
const applyFilter = async () => {
  ...
  if (activeFilter.value.wilayah_id) params.wilayah_id = activeFilter.value.wilayah_id
  if (activeFilter.value.lingkungan_id) params.lingkungan_id = activeFilter.value.lingkungan_id
  if (activeFilter.value.seksi_id) params.seksi_id = activeFilter.value.seksi_id
...
const clearFilter = async () => {
  activeFilter.value = { wilayah_id: null, lingkungan_id: null, seksi_id: null, is_bgkp: false }
  fetchParams.value = {}
  currentPage.value = 1
  await refresh()
}
