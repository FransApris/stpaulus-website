<template>
      ...
      <div class="px-4 py-5 sm:p-6">
        <div class="flex justify-between items-center mb-4">
          ...
          <div class="flex space-x-2">
            <select v-model="filterStatus" @change="fetchNews"
        ...
        <div v-else class="space-y-4">
          <div v-for="newsItem in news" :key="`news-${newsItem.id}-${newsItem.status}`"
            class="border border-gray-200 rounded-lg p-4 hover:bg-gray-50">
            <div class="flex justify-between items-start gap-4">
              <!-- Thumbnail -->
              <div v-if="newsItem.image" class="flex-shrink-0">
                <img :src="newsItem.image" :alt="newsItem.title" class="w-32 h-24 object-cover rounded-lg" />
              </div>
              ...
              <div class="flex-1 min-w-0">
                ...
                <div class="flex items-center space-x-4 mt-2 text-sm text-gray-500">
                  ...
                  <span :class="getStatusClass(newsItem.status)" class="px-2 py-1 rounded-full text-xs font-medium">
                    {{ getStatusText(newsItem.status) }}
                  </span>
                </div>
              </div>
              <div class="flex space-x-2 flex-shrink-0">
                <button @click="editNews(newsItem)" title="Edit"
                ...
                <button @click="togglePublish(newsItem)"
                  ...
                  <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    <path v-if="newsItem.status === 'published'" stroke-linecap="round" stroke-linejoin="round"
                      stroke-width="2"
                ...
                <button @click="deleteNews(newsItem)" title="Hapus"
  ...
  <!-- Add/Edit News Modal -->
  <div v-if="showAddModal || editingNews"
      ...
      <div class="mt-3">
        ...
        <form @submit.prevent="saveNews" class="space-y-4">
          ...
          <div>
            ...
            <input v-model="newsForm.slug" type="text" required readonly disabled
          ...
          <div>
            ...
            <textarea v-model="newsForm.excerpt" rows="3"
              class="mt-1 block w-full border border-gray-300 rounded-md shadow-sm py-2 px-3 focus:outline-none focus:ring-[#882f1d] focus:border-[#882f1d]"></textarea>
          </div>
          ...
          <div class="border-t border-gray-200 pt-4">
            ...
            <div class="space-y-3">
              ...
              <div class="grid grid-cols-1 md:grid-cols-2 gap-3">
                <div>
                  ...
                  <input v-model="newsForm.when_date" type="date"
                    class="mt-1 block w-full border border-gray-300 rounded-md shadow-sm py-2 px-3 focus:outline-none focus:ring-[#882f1d] focus:border-[#882f1d]" />
                </div>
                <div>
                  <label class="block text-sm font-medium text-gray-700">🕐 Jam</label>
                  <input v-model="newsForm.when_time" type="text" placeholder="08.00 - 12.00 WIB"
                    class="mt-1 block w-full border border-gray-300 rounded-md shadow-sm py-2 px-3 focus:outline-none focus:ring-[#882f1d] focus:border-[#882f1d]" />
                </div>
              </div>
              ...
              <div>
                ...
                <textarea v-model="newsForm.where_location" rows="2" placeholder="Gereja Paroki St. Paulus"
                  class="mt-1 block w-full border border-gray-300 rounded-md shadow-sm py-2 px-3 focus:outline-none focus:ring-[#882f1d] focus:border-[#882f1d]"></textarea>
              </div>
              ...
              <div>
                ...
                <textarea v-model="newsForm.who_participants" rows="2" placeholder="Seluruh umat paroki"
                  class="mt-1 block w-full border border-gray-300 rounded-md shadow-sm py-2 px-3 focus:outline-none focus:ring-[#882f1d] focus:border-[#882f1d]"></textarea>
              </div>
              ...
              <div>
                ...
                <textarea v-model="newsForm.why_purpose" rows="2" placeholder="Merayakan..."
                  class="mt-1 block w-full border border-gray-300 rounded-md shadow-sm py-2 px-3 focus:outline-none focus:ring-[#882f1d] focus:border-[#882f1d]"></textarea>
              </div>
              ...
              <div>
                ...
                <textarea v-model="newsForm.how_process" rows="2" placeholder="Dimulai dengan..."
                  class="mt-1 block w-full border border-gray-300 rounded-md shadow-sm py-2 px-3 focus:outline-none focus:ring-[#882f1d] focus:border-[#882f1d]"></textarea>
              </div>
            </div>
            ...
            <!-- AI Generate Button -->
            <div class="mt-4">
              <button type="button" @click="generateNarasi" :disabled="aiGenerating || !newsForm.title"
          ...
          <div>
            ...
            <ClientOnly>
              <LazyCKEditorWrapper v-model="newsForm.content" placeholder="Tulis konten berita di sini..." />
              <template #fallback>
                <div
          ...
          <AdminImageUpload v-model="newsForm.image" label="Gambar/Thumbnail Berita"
            helper-text="Gambar akan tampil sebagai thumbnail di halaman utama dan detail berita" type="news" />
          ...
          <div>
            ...
            <div class="mt-1 max-h-32 overflow-y-auto border border-gray-300 rounded-md p-2">
              <div v-for="category in allCategories" :key="category.id" class="flex items-center">
                <input :id="`category-${category.id}`" v-model="newsForm.category_ids" :value="category.id"
                  type="checkbox" class="h-4 w-4 text-[#882f1d] focus:ring-[#882f1d] border-gray-300 rounded" />
          ...
          <div class="border-t border-gray-200 pt-4">
            ...
            <div class="mb-3">
              ...
              <div class="max-h-28 overflow-y-auto border border-gray-200 rounded-md p-2 grid grid-cols-2 gap-1">
                <label v-for="w in allWilayah" :key="w.id"
                  class="flex items-center gap-2 cursor-pointer hover:bg-gray-50 rounded px-1 py-0.5">
                  <input type="checkbox" :value="w.id" v-model="newsForm.wilayah_ids"
            ...
            <div class="mb-3">
              ...
              <div class="max-h-28 overflow-y-auto border border-gray-200 rounded-md p-2 grid grid-cols-2 gap-1">
                <label v-for="l in allLingkungan" :key="l.id"
                  class="flex items-center gap-2 cursor-pointer hover:bg-gray-50 rounded px-1 py-0.5">
                  <input type="checkbox" :value="l.id" v-model="newsForm.lingkungan_ids"
                    class="h-3.5 w-3.5 text-[#882f1d] rounded border-gray-300" />
                  <span class="text-xs text-gray-700">{{ l.nama }}</span>
                </label>
              </div>
            </div>
            ...
            <!-- Seksi -->
            <div class="mb-3">
              <label class="block text-sm font-medium text-gray-700 mb-1">Seksi / Bidang</label>
              <div class="max-h-36 overflow-y-auto border border-gray-200 rounded-md p-2 space-y-2">
                <div v-for="bidang in seksiGrouped" :key="bidang.nama">
                  <p class="text-xs font-semibold text-gray-500 uppercase tracking-wide px-1 mb-1">{{ bidang.nama }}</p>
                  <div class="grid grid-cols-2 gap-1">
                    <label v-for="s in bidang.seksi" :key="s.id"
                      class="flex items-center gap-2 cursor-pointer hover:bg-gray-50 rounded px-1 py-0.5">
                      <input type="checkbox" :value="s.id" v-model="newsForm.seksi_ids"
                        class="h-3.5 w-3.5 text-[#882f1d] rounded border-gray-300" />
            ...
            <div>
              <label class="flex items-center gap-3 cursor-pointer w-fit">
                <input type="checkbox" v-model="newsForm.is_bgkp"
                  class="h-4 w-4 text-[#882f1d] rounded border-gray-300" />
                <span class="text-sm font-medium text-gray-700">Tandai sebagai berita <strong>BGKP</strong> (Badan
                  Gereja Katolik
                  Paroki)</span>
              </label>
            </div>
...
const fetchOrganizationData = async () => {
  try {
    ...
    allWilayah.value = wilayahRes?.data || wilayahRes || []
    allLingkungan.value = Array.isArray(lingkunganRes) ? lingkunganRes : []
    allSeksi.value = Array.isArray(seksiRes) ? seksiRes : []
  } catch (error) {
