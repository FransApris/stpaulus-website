    ...
    <div class="bg-white rounded-lg shadow-sm overflow-hidden">
      ...
      <div v-else>
        ...
        <div class="md:hidden space-y-4 p-4 bg-gray-50">
          <div v-for="category in paginatedCategories" :key="`mobile-${category.id}`" class="bg-white border border-gray-200 rounded-xl p-4 shadow-sm relative">
            <div class="flex justify-between items-start mb-3">
              <div class="flex items-center gap-3">
                <div class="flex-shrink-0 w-8 h-8 rounded-lg shadow-sm border border-gray-100" :style="{ backgroundColor: category.color }"></div>
                <div>
                  <h4 class="text-[17px] font-bold text-gray-900 leading-tight">{{ category.name }}</h4>
                  <p class="text-xs text-gray-500 mt-0.5">{{ category.slug }}</p>
                </div>
              </div>
              <span class="inline-flex items-center px-2 py-0.5 rounded-full text-[11px] font-bold bg-blue-50 text-blue-700 border border-blue-100 uppercase tracking-wide">
                {{ category.agenda_count || 0 }} agenda
              </span>
            </div>
            
            <p class="text-sm text-gray-600 mb-4 line-clamp-2">{{ category.description || 'Tidak ada deskripsi' }}</p>
            
            <div class="flex items-center gap-2 pt-3 border-t border-gray-100">
              <button
                @click="openPreview(category)"
                :disabled="deleting === category.id"
                class="flex-1 bg-indigo-50 text-indigo-700 hover:bg-indigo-600 hover:text-white border border-indigo-200 hover:border-transparent py-2.5 rounded-lg text-sm flex justify-center items-center transition-colors"
                title="Lihat Preview"
              >
              ...
              <button
                @click="openModal(category)"
                :disabled="deleting === category.id"
                class="flex-1 bg-orange-50 text-[#882f1d] hover:bg-orange-600 hover:text-white border border-orange-200 hover:border-transparent py-2.5 rounded-lg text-sm flex justify-center items-center transition-colors disabled:opacity-50"
                title="Edit"
              >
                <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                  <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z"></path>
              ...
              <button
                @click="showDeleteConfirm(category)"
                :disabled="deleting === category.id"
                class="flex-1 bg-red-50 text-red-700 hover:bg-red-600 hover:text-white border border-red-200 hover:border-transparent py-2.5 rounded-lg text-sm flex justify-center items-center transition-colors disabled:opacity-50"
                title="Hapus"
              >
                <svg class="w-4 h-4" :class="deleting === category.id ? 'animate-spin' : ''" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                  <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 7l-.867 12.142A2 2 0 0116.138 21H7.862a2 2 0 01-1.995-1.858L5 7m5 4v6m4-6v6m1-10V4a1 1 0 00-1-1h-4a1 1 0 00-1 1v3M4 7h16"></path>
                </svg>
              </button>
            </div>
