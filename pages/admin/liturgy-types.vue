  ...
  <div class="space-y-6">
    ...
    <!-- ── Desktop: Tabel standar (md ke atas) ── -->
      ...
      <div class="md:hidden space-y-3 p-4">
        <div v-for="type in liturgyTypes" :key="type.id"
          class="bg-white border border-gray-200 rounded-lg p-4 shadow-sm"
          :class="{ 'opacity-60 animate-pulse': type._isOptimistic }">
          <!-- Header kartu: ikon + nama + badge status -->
          <div class="flex items-start justify-between mb-3">
            <div class="flex items-center gap-3">
              ...
              <div>
                ...
                <p class="text-xs text-gray-400 mt-0.5">{{ type.slug }}</p>
              </div>
            </div>
            <span class="text-xs font-semibold px-2 py-0.5 rounded-full shrink-0 ml-2"
              :class="type.is_active ? 'bg-green-100 text-green-800' : 'bg-red-100 text-red-800'">
              {{ type.is_active ? 'Aktif' : 'Nonaktif' }}
            </span>
          </div>
          ...
          <div class="space-y-1.5 text-sm border-t border-gray-100 pt-3">
            <div class="flex justify-between">
              ...
              <span class="text-gray-800 text-right max-w-[60%]">{{ type.description || '-' }}</span>
            </div>
            <div class="flex justify-between">
              <span class="text-gray-500">Urutan Tampil</span>
              <span class="text-gray-800">{{ type.display_order }}</span>
            </div>
          </div>
          <!-- Tombol aksi -->
          <div class="flex gap-2 mt-3 pt-3 border-t border-gray-100">
            <button @click="editType(type)" :disabled="type._isOptimistic || loading"
              class="flex-1 flex items-center justify-center gap-1.5 px-3 py-2 bg-indigo-50 text-indigo-700 hover:bg-indigo-100 text-xs font-medium rounded-lg transition-colors disabled:opacity-50">
            ...
            <button @click="confirmDelete(type)" :disabled="type._isOptimistic || loading"
              class="flex-1 flex items-center justify-center gap-1.5 px-3 py-2 bg-red-50 text-red-700 hover:bg-red-100 text-xs font-medium rounded-lg transition-colors disabled:opacity-50">
              <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 7l-.867 12.142A2 2 0 0116.138 21H7.862a2 2 0 01-1.995-1.858L5 7m5 4v6m4-6v6m1-10V4a1 1 0 00-1-1h-4a1 1 0 00-1 1v3M4 7h16"></path>
              </svg>
              Hapus
