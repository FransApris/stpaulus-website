<template>
  <div>
    ...
    <div id="document-categories-section" class="bg-white rounded-xl shadow-sm border border-gray-100 overflow-hidden scroll-mt-4">
      ...
      <div v-else>
        ...
        <div class="xl:hidden p-3 sm:p-4 space-y-4">
          <div v-for="category in paginatedCategories" :key="'card-'+category.id" class="bg-white border border-gray-200 rounded-xl p-4 sm:p-5 shadow-sm hover:shadow-md transition-shadow">
            <!-- Card Header: Nama Kategori & Slug (Lebar Penuh) -->
            <div class="mb-3.5 pb-3 border-b border-gray-100">
              <div class="flex items-center gap-2.5">
                <div class="flex-shrink-0 w-3.5 h-3.5 rounded-full shadow-sm" :style="{ backgroundColor: category.color }"></div>
...
const scrollToTop = () => {
  nextTick(() => {
    const target = document.getElementById('document-categories-section')
    if (target) {
